...
export class SupabaseStorageAdapter implements StorageAdapter {
  ...
  async createSignedUploadUrl(bucket: string, path: string, _ttlSec: number): Promise<SignedUpload> {
    // Supabase signed upload URLs are valid for 2 hours; ttlSec is enforced on our side via expiresAt.
...
export class MemoryStorageAdapter implements StorageAdapter {
  ...
  put(bucket: string, path: string, data: Buffer, contentType: string): void {
    this.objects.set(this.key(bucket, path), { data, contentType, updatedAt: new Date() });
  }

  async createSignedUploadUrl(bucket: string, path: string, _ttlSec: number): Promise<SignedUpload> {
    const token = `upload-token-${this.signedUploads.size + 1}`;
    this.signedUploads.set(token, { bucket, path });
    return { signedUrl: `memory://upload/${bucket}/${path}?token=${token}`, token, path };
  }

...
제품 범위의 최상위 기준은 사용자가 확정한 3절이다. 이후 절의 기술 설계는 이를 실현하기 위한 구현 기준이며 새로운 사업 요구를 추가하지 않는다. 사용자 지시와 적용되는 AGENTS.md를 확인하고 기존 작업을 보존한다.
...
구현 순서는 입력·업로드 검증 → 실제 AI 추출 → 사용자의 초안 확인·수정 → 제출·공개다. AI가 초안을 자동 게시하지 않는다. 추출 실패 시 원본을 보존하고 오류와 재시도 가능 여부를 제공하며, 수동으로 내용을 보완해 등록할 수 있다. 사진, 발견 장소, 상세 설명은 습득 등록 필수 입력이다.

### 3.2 찾기

분실자가 사진 또는 설명, 마지막 위치·시간을 제출하고 초안을 확인한다. 그 이후, 사용자가 광고를 시청하고 서버가 시청 완료를 검증한 이후에, 실제 AI를 통해서 검색을 실행한다.

사용자의 “서버가 광고를 시청한 이후”라는 표현은 사용자가 광고를 시청하고 서버가 완료를 확인한다는 의미로 구현한다. 서버가 광고를 재생하거나 시청하는 것으로 구현하지 않는다.

검색 소유권과 광고 세션 연결을 위해 찾기 요청도 로그인한 사용자를 기준으로 저장한다. 사진은 없어도 되지만 사진과 설명이 모두 없으면 거절한다. 마지막 위치와 시간은 필수다. 광고 전 초안 확인은 입력값 저장·검증으로 수행하고, 분실 요청에 대한 AI 분석과 후보 매칭은 광고 검증 후 시작한다. 기존 습득물 AI 정보 추출은 무료 등록 흐름이므로 이 광고 조건과 별개다.
...
광고 공급자는 아직 선정된 것으로 가정하지 않는다. 실제 구현 시 웹에서 사용할 수 있는 광고 형식과 서버 검증 기능을 확인하여 AdsProvider adapter를 연결한다. 공급자가 지원하지 않는 검증 API를 만들어 있다고 가정하지 않는다.

1. 사용자가 검색 초안을 확인하면 서버가 해당 입력 버전과 sourceHash를 고정한다.
2. 서버는 ownerId, searchRequestId, sourceHash에 연결된 광고 세션과 추측 불가능한 식별자를 생성한다.
3. FE는 공급자 광고를 사용자에게 보여 준다.
4. 서버는 공급자의 서명된 콜백 또는 서버 간 조회 등 검증 가능한 완료 증거를 확인한다.
5. 세션·사용자·광고 단위·완료 결과·유효기간이 일치할 때만 VERIFIED로 바꾼다.
6. FE의 검색 실행 요청에서 이 검증된 세션을 한 번 소비하고 검색 작업을 만든다.

단순 버튼 클릭, 프런트 타이머, watched=true, 클라이언트만 발생시킨 종료 이벤트를 실제 검증으로 인정하지 않는다. 콜백 서명·재전송 방지는 공급자 명세에 맞게 구현하고 provider event ID 중복을 DB UNIQUE로 차단한다. raw body 서명이 필요한 공급자는 JSON 변환 전에 검증한다.

광고 미완료·취소·만료·검증 실패 상태에서는 AI 검색 작업이 없어야 한다. 공급자 장애나 광고 재고 없음은 완료로 위장하지 않고 이유와 재시도 가능 여부를 노출한다. 공급자가 서버에서 검증 가능한 증거를 제공하지 않으면 실제 연동 미완료로 기록하고 capabilities.ads=false로 표시한다.

세션 유효기간은 공급자 정책에 맞는 ADS_SESSION_TTL_SECONDS 설정으로 정의하고 API에서 expiresAt을 반환한다. 만료되거나 소비된 세션을 재사용할 수 없다. 초안 수정 시 기존 미소비 세션을 무효화한다. 입력이 바뀐 후 도착한 이전 콜백으로 새 입력을 승인하지 않는다.

ADS_MODE=demo는 명시적 개발·테스트 기능이다. demo 완료 경로는 본인 세션에만 적용하고 live 환경에는 등록하지 않는다. 응답에서 demo임을 밝히며 실제 광고 검증 완료로 보고하지 않는다.
...
습득 등록은 검증된 사진과 상세 설명에서 제목·카테고리·색상·특징을 추출한다. 사용자 입력과 이미지 속 문자는 데이터로 취급하고 모델에 비밀 조회나 권한 변경 도구를 주지 않는다. 구조화 응답을 런타임 검증하고 없는 특징이나 연락처·보관 장소를 생성하지 않는다. 사용자가 수정한 값을 자동 덮어쓰지 않는다.

검색은 광고 검증 후 실제 모델을 호출하는 경로를 완성한다. 공개 가능한 AVAILABLE 습득물을 대상으로 사진·설명·마지막 위치·시간과 후보 정보를 비교한다.

- 사진이 있으면 실제 이미지 입력을 사용하는 multimodal 비교 또는 이미지 embedding을 사용한다.
- 설명은 의미 비교 또는 텍스트 embedding으로 반영한다.
- 위치는 거리, 시간은 알려진 발견 시각과의 관계를 보조 신호로 사용한다. 발견 시각이 없으면 결측 처리한다.
- 사진이 없는 요청도 설명으로 검색할 수 있어야 한다. 설명이 없는 사진 요청도 정상 처리한다.
- 후보 ID만 모델에 주고 선택시키거나 고정 fixture 순위를 실제 AI 결과로 반환하지 않는다.
- 반환 점수는 순위 지표이며 소유권이나 정답 확률의 증명이 아니다. 일치 근거는 실제 공개 입력에 근거한다.

후보 상한, 가중치, 임계값, timeout, 동시 호출 수는 설정과 모델 버전으로 관리하고 작은 평가 fixture로 검증한다. 등록된 후보가 없으면 외부 AI를 불필요하게 호출하지 않고 COMPLETED와 빈 결과를 반환할 수 있다. 외부 AI 장애는 FAILED로 구분하며 후보 없음으로 숨기지 않는다.

원본 수정 시 sourceHash로 추출 정보를 무효화하고 이전 작업이 새 내용을 덮어쓰지 못하게 한다. 후보 feature는 동일 모델·입력 버전에서 캐시할 수 있다. 후보 조회 시 현재 공개 상태를 다시 확인하여 보관 처리된 물건이나 삭제된 공개 정보를 노출하지 않는다.

AI_MODE=demo는 결정적인 fixture 테스트용이다. live 실패 시 자동 전환하지 않는다. 실제 AI MVP 완료는 실제 공급자 호출을 통한 습득 정보 추출과 광고 확인 후 검색의 성공 증거를 포함해야 한다. 키가 없으면 로컬 검증은 수행하되 실제 AI 연동·검증 미완료를 명시한다.
...
| POST /found-items/:id/analyses | 작성자; 현재 초안의 실제 AI 추출 접수, 202 Analysis |
...
| POST /search-requests/:id/ad-sessions | 본인; 확정된 입력에 연결된 세션과 공급자 표시용 정보 발급 |
| GET /ad-sessions/:id | 본인; 상태·expiresAt 조회 |
| POST /webhooks/ads | 공급자 검증 후 완료 이벤트를 durable 저장; 중복 이벤트 멱등 처리 |
| POST /search-requests/:id/execute | 본인 {adSessionId}; 검증된 세션 소비 후 202 실행 정보 |
...
| POST /demo/ad-sessions/:id/complete | demo 전용, 본인; 테스트 완료 처리 |
| GET /health | 공개 최소 liveness |
| GET /ready | 인프라 제한; DB·migration·필수 설정 readiness |

공급자 검증 방식이 서버 간 조회라면 광고 세션 상태 조회에서 provider 확인을 수행할 수 있다. 공급자 내부 필드를 공통 DTO에 무분별하게 노출하지 않는다. /found-items/map은 :id 경로보다 먼저 등록한다.
...
주요 오류는 400 VALIDATION_ERROR, 401 UNAUTHENTICATED, 403 AD_NOT_VERIFIED, 404 NOT_FOUND, 409 INVALID_STATE/VERSION_CONFLICT/IDEMPOTENCY_CONFLICT/REQUEST_IN_PROGRESS, 410 AD_SESSION_EXPIRED, 413 FILE_TOO_LARGE, 415 UNSUPPORTED_MEDIA_TYPE, 429 RATE_LIMITED, 503 PROVIDER_UNAVAILABLE이다. 서명 검증 실패 광고 콜백은 401을 반환한다.

## 11. 운영과 환경 설정

요청별 인증·rate limit·body 크기·timeout을 적용한다. CORS는 지정 origin만 허용하고 공개 health에는 내부 경로나 비밀을 넣지 않는다. worker는 지수 backoff와 최대 재시도 수를 적용한다. 필수 작업은 습득물 AI 추출·feature 처리, 광고 검증된 검색, 미연결 업로드와 만료 광고 세션 정리다.

requestId, 최소 actorId, runId, 소요시간, 결과 코드와 AI 공급자·모델·호출 횟수를 기록한다. 광고 검증 실패, AI 실패·지연, outbox 대기, 재시도 횟수를 관측한다. DB와 업로드의 백업·복원 방법을 README에 적고 서버 재시작 후 진행 중 작업이 복구되도록 한다.
...
| 시나리오 | 통과 기준 |
| --- | --- |
| 습득 등록 | 로그인·사진·발견 장소·상세 설명 검증, 실제 AI 추출, 사용자 확인 후 공개 |
...
| live 설정 누락 | demo로 위장하지 않고 미지원·오류를 표시 |

테스트는 임시 DB와 fake clock/provider를 사용한다. 광고 소비 경합은 같은 SQLite 파일에 별도 connection을 사용하여 검증한다. 실제 AI 검증은 fake 테스트와 별도로 실행 결과를 남긴다. 테스트 결과만 있는 demo와 실제 공급자 연동 검증을 구분한다.
...
4. 습득 등록의 실제 AI 추출·초안 확인·게시와 목록·지도 API를 구현한다.
5. 검색 초안·확정, 광고 세션·완료 검증, 원자적 검색 실행을 구현한다.
6. 실제 AI 매칭·결과 조회·오류 재시도와 worker 복구를 연결한다.
7. API·권한·경합 테스트, typecheck, build, 재시작을 검증한다.
8. FE와 동일한 demo fixture 및 새 계약을 공유하고 실제 API로 세 사용자 흐름을 통합 검증한다.
9. 실행법, 필요한 외부 설정, 실제 공급자 검증 결과와 남은 항목을 README에 기록한다.

MVP 완료 조건:

- 로그인한 습득자의 입력 → 실제 AI 추출 → 초안 확인 → 공개 등록이 동작한다.
- 분실 요청 → 초안 확인 → 광고 완료 서버 검증 → 실제 AI 검색 → 결과 조회가 동작한다.
- 지도 API가 공개 가능한 위치를 제공하고 FE에서 목록과 같은 물건을 지도에 표시할 수 있다.
- 비공개 보관 장소·연락처·타인의 마지막 위치가 공개 API에 노출되지 않는다.
- 광고 검증 우회, 중복 검색, 서버 재시작과 AI 장애에 대한 검증이 통과한다.
- OpenAPI, migration, .env.example, 명시적 demo seed, 실행 README가 제공된다.
- demo만 완성했거나 실제 광고·AI 연결이 미검증이면 전체 MVP 완료로 보고하지 않고 남은 조건을 명시한다.

## 14. 백엔드 구현 에이전트에 전달할 프롬프트

```text
Lumina 백엔드 MVP를 구현하고 검증하라. 사용자 확정 흐름인 BE_MASTER.md 3절을 절대적인 제품 기준으로 삼아라.

범위는 무료 습득 등록, 광고 시청 완료 서버 검증 후 실제 AI 찾기, 지도용 위치 데이터 제공이다. 로그인한 습득자의 사진·발견 장소·상세 설명을 검증하고 실제 AI로 제목·카테고리·색상·특징을 추출한 뒤 사용자 확인을 거쳐 게시하라. 보관 장소 상세와 연락처는 비공개로 저장하라.

분실자는 사진 또는 설명과 마지막 위치·시간을 입력하고 초안을 확인한다. 사용자의 광고 시청 완료를 서버에서 검증한 후에만 AI 검색을 실행하라. 광고 세션을 사용자·검색·입력 버전에 연결하고 단일 소비와 실행 생성을 transaction으로 보장하라. 실패 재시도는 같은 실행을 사용하고 광고를 다시 요구하지 마라.
